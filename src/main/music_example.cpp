...
void drawMusicExample()
{
    ...
    static Music music = LoadMusicStream("resources/country.mp3");

    if (init)
      PlayMusicStream(music);

    static float timePlayed = 0.0f;        // Time played normalized [0.0f..1.0f]
    static bool pause = false;             // Music playing paused

    static float pan = 0.0f;               // Default audio pan center [-1.0f..1.0f]
    if (init)
      SetMusicPan(music, pan);

    static float volume = 0.8f;            // Default audio volume [0.0f..1.0f]
    if (init)
